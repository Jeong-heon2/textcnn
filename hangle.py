...
def normalize(doc, english=False, number=False, punctuation=False, remove_repeat=0, remains={}):
    ...
    for c in doc:
        ...
        if (c == ' ') or (is_korean(i)) or (english and is_english(i)) or (number and is_number(i)) or (
                punctuation and is_punctuation(i)):
            f += c
        elif c in remains:
            f += c
        else:
...
class ConvolutionalNN_Encoder:
    ...
    def __init__(self, vocabs={}):
        ...
        self.cvocabs_ = [' ', 'ㄱ', 'ㄲ', 'ㄳ', 'ㄴ', 'ㄵ', 'ㄶ', 'ㄷ', 'ㄸ', 'ㄹ',
                         ...
                         'ㅢ', 'ㅣ']
        self.cvocabs = {}
        self.cvocabs = {c: len(self.cvocabs) + 1 for c in self.cvocabs_}
